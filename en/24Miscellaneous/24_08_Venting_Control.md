...
*SetP Co2* is the set- point for CO<sub>2</sub> concentration (ppm) in the indoor air. If "0" (zero) is given the natural ventilation will not be controlled according to the calculated CO<sub>2</sub> level. Above the set point venting will come into action as much as needed to fulfill the set point. If SetP Co2 is given (> 0) the natural ventilation is first controlled to obtain the desired CO<sub>2</sub> concentration, and then to obtain the temperature set point.
...
To reach the desired set-point for the CO<sub>2</sub> content in the indoor air, the flow of outdoor air is controlled by the [basic air change and the wind part](../11Systems/11_18_Systems_Venting.md) in the equation.
