...
Using the *fan coil control* it is possible to control a local cooling system in BSim, taking into account that the cooling power is dependent on the temperature difference between the zone temperature and the cooling surface (the cooling fluid). This cooling control can be used when modeling *fan coils*.
...
*   *Θ<sub>dim </sub>* is the dimensioning zone temperature for the local cooling system (the temperature over which the system reaches its dimensioning cooling power).
