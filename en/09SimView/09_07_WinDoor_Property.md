...
The message *Impossible Geometry* may also be caused by a wrong location of the actual Windoor, stretching into the *Inner Shell*, i.e. the constructions of the faces that meet the face that contains the Windoor. This can occur if the thickness of the constructions is increased, e.g. by selecting new constructions from the database. It is not possible to run a simulation when impossible geometry is shown for any Windoor.
...
*   *DayLight*: Here three [solar light factors](../10Thermal_zones/10_07_Solar_light_factors_for_WinDoors.md) can be defined for the window ([Sf1](../10Thermal_zones/10_07_Solar_light_factors_for_WinDoors.md), [Sf2](../10Thermal_zones/10_07_Solar_light_factors_for_WinDoors.md) and [Sf3](../10Thermal_zones/10_07_Solar_light_factors_for_WinDoors.md)) manually. The solar light factors can be calculated and transferred automatically to the WinDoor dialog from [SimLight](../15SimLight_Daylight_calculations/15_02_Daylight_calculation_at_a_point.md).

*   *Side and Overhang*: The local geometry surrounding the object can be described by clicking the *Left, Right and Top buttons*, which open [dialog boxes](../24Miscellaneous/24_54_WinDoor_overhangs_and_side-fins.md) for defining the geometry of side fins and overhangs.  
<span id="red_text"> **NB:** Overhang and Sidefin **now** work in combination with [XSun solar distribution](../13tsbi5_thermal_simulation/13_02_tsbi5_options.md) but are not shown in [XSun](../14XSun_Analysis_of_incident_solar_radiation/14_01_Analysis_of_incident_solar_radiation_with_XSun.md) visualizations. If an overhang or side-fin is to be simulated using the XSun solar distribution these obstacles should preferably be defined geometrically in the model.   
If Overhang or Sidefin is used **in combination with** XSun solar distribution, there is a risk of double shading on WinDoor and thus very reduced solar incidence on the model. </span>

Based on this information the software calculates the frame area, panel area and the area of the caulking joint. The total U-value for a *WinDoor* is calculated in accordance with the instructions contained in the appendix to [DS418](../20The_Mathematical_basis/20_28_Literature.md) relating to the calculation of heat loss through windows and doors.
