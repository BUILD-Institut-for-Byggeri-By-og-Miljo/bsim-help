...
On the basis of equation (1) the heat flux is calculated from the temperatures at the end of the time-step. When the calculation has arrived at time-step j, and the conditions are to be calculated up to the time j+1, the condition at the conclusion of this time-step are not yet known, and one cannot therefore insert explicit values for the temperatures on the right hand side of the equation (1). An implicit calculation procedure is therefore used, as described below.

Throughout the time-step, the implicitly indicated heat flux from equation (2) is assumed to be constant, and the increase in enthalpy for the control volume i is summed up as follows:
...
By expressing the specific enthalpy alterations as a temperature alteration multiplied by the specific heat capacity, and by inserting the expressions for q from equation (1) the following expression can be set up:


$$
\begin{aligned}
(\rho c_p)_i \Delta x_i
\frac{T_i^{j+1} - T_i^j}{\Delta t}
&=
\frac{T_{i-1}^{j+1} - T_i^{j+1}}
{\dfrac{\Delta x_{i-1}}{2 \lambda_{i-1}}
+ \dfrac{\Delta x_i}{2 \lambda_i}
+ R_i} \\
&\quad
+ \frac{T_{i+1}^{j+1} - T_i^{j+1}}
{\dfrac{\Delta x_i}{2 \lambda_i}
...
This equation is set up for all i and is solved simultaneously for all, as the equations mutually use "each other's" information regarding the temperature on the new time level. This is only possible if the boundary conditions are known. Equation (3) is therefore expressed in a slightly different form, which is valid for the boundary volume, i= 1, at face 1 of the construction. As the boundary node lies close to the actual boundary, the following equation (4) is achieved:


$$
\begin{aligned}
(\rho c_p)_1 \Delta x_1
\frac{T_1^{j+1} - T_1^j}{\Delta t}
&=
q_{\text{surf,side1}}
+ \frac{T_{\text{air}} - T_1^{j+1}}{R_{\text{surf side 1}}} \\
&\quad
+ \frac{T_2^{j+1} - T_1^{j+1}}
{\dfrac{\Delta x_1}{\lambda_1}
+ \dfrac{\Delta x_2}{2 \lambda_2}
+ R_2}
\end{aligned}
\qquad (4)
$$
...
Equation (4) gives no indication of what time level is valid for the neighboring zone's air temperature T<sub>air</sub>. This question is dealt with at the end of this section.
...
Expressions corresponding to these are valid for the H-functions (H<sub>n</sub> and H<sub>n1</sub>) close to the surface at face 2. By means of these help-values, equations (3) and (4) can be rewritten in a simpler form, as links which involve the temperatures on a "new" time level, j+1, are written on the left hand side of the equals sign, and values given explicitly, such as links which involve the temperature on the "old" time level, j, are written on the right hand side.
...
This expression can be further simplified by introducing the coefficients A, B, C and D, the definition of which is immediately apparent by comparison between equation 8 and equation 9:
...
The expressions from equations 11, 12 and the corresponding expression which could be written for the surface at face 2, can be gathered into one equation system:
