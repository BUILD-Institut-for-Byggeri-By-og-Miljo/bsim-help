...
$$ = \frac{1}{4} \int_{\partial A_{1}} \int_{\partial A_{2}} \ln (\vec{r} \cdot \vec{r}) d \vec{x}_{2} \cdot d \vec{x}_{1} \tag{1} $$
...
$$ 4 \pi A_{P_{1}} F_{P_{1} P_{2}}=\sum_{E_{i} E_{j}} \cos \angle E_{i} E_{j} \int_{E_{i}} \int_{E_{j}} \ln (\vec{r} \cdot \vec{r}) d s_{j} d t_{i} \tag{2} $$
...
$$ = \left.\left\{\left.\left[\left(s+\frac{c_{3}}{2}\right) \mathrm{G}(f(., t))(s)+\frac{c_{1}}{2} \mathrm{H}(f(., t))(s)\right]\right|_{s=0} ^{s=c_{0}}\right\}\right|_{t=0} ^{t=c_{2}} $$


$$
-2 c_{0} c_{2} + c_{14} c_{15} \Bigl[ \pi \bigl( 2 k(s) + 1 \bigr) \, \mathrm{M}(t) \Bigr]
$$

$$ 
-i\left(\mathrm{~L}\left(-c_{17}(s)\right)(t)+\mathrm{L}\left(-c_{18}(s)\right)(t)\right. 
$$  
...
A simple example, which requires the full power of our formula, concerns the form factor between two equal width rectangles sharing an edge with an enclosing angle θ $ \in $ [0; π]. The configuration is illustrated in Figure 1 together with the form factor as a function of θ for different aspect ratios l = a/b (common edge length b).
  ...
  $$
\begin{aligned}
\mathrm{L}(b)(y) &:= \int^y t^2 (1-t^2)^{-3} \ln(b+t)\, \mathrm{d}t \\[2mm]
&= \frac{1}{16} \Bigg[
  \frac{-b \ln(y-1)}{(b+1)^2} 
  - \frac{b \ln(1+y)}{(b-1)^2} \\[1mm]
&\quad + \Bigg(
      \frac{2(b+y)(1+b y)((b-y)^2 + (b y-1)^2)}
           {(b^2-1)^2 (y^2-1)^2} 
      + \ln \frac{(1-y)(1-b)}{(1+y)(1+b)}
    \Bigg) \ln(b+y) \\[1mm]
&\quad + \frac{2(b-y)}{(b^2-1)(y^2-1)} 
  + \mathrm{Li}_2\!\Big(\frac{1-y}{1+b}\Big) 
  - \mathrm{Li}_2\!\Big(\frac{1+y}{1-b}\Big)
\Bigg]
\end{aligned}
\qquad (4)
...
$$
\begin{aligned}
\mathrm{M}(y) &:= \int^y t^2 (1-t^2)^{-3} \, dt \\[1mm]
&= \frac{1}{16} \Bigg[
    4 y (y^2-1)^{-2} + 2 y (y^2-1)^{-1} 
    + \ln \frac{y-1}{y+1}
\Bigg]
\end{aligned}
\qquad (5)
...
$$
\begin{aligned}
\mathrm{G}(q)(y) &:= \int^y \ln q(t) \, dt \\[1mm]
&= \frac{q'(y)}{2 a} \ln q(y) - 2 y 
   + \frac{d}{a} \tan^{-1} \frac{q'(y)}{d}
\end{aligned}
\qquad (6)
$$
...
$$
\begin{aligned}
\mathrm{H}(q)(y) &:= \int^y t \ln q(t) \, dt \\[1mm]
&= \left( \frac{y^2}{2} + \frac{c}{2a} - \frac{b^2}{4 a^2} \right) \ln q(y) 
   - \frac{y(a y - b)}{2 a} 
   - \frac{b d}{2 a^2} \tan^{-1} \frac{q'(y)}{d}
\end{aligned}
\qquad (7)
$$
...
$$ c_1 = -2 \vec{d}_i \cdot \vec{d}_j \tag{9} $$

$$ c_2 = ||E_i|| \tag{10} $$

$$ c_3 = -2 \vec{d}_j \cdot (\vec{p}_i - \vec{p}_j) \tag{11} $$

$$ c_4 = 2 \vec{d}_i \cdot (\vec{p}_i - \vec{p}_j) \tag{12} $$

$$ c_5 = ||\vec{p}_i - \vec{p}_j||^2 \tag{13} $$

$$ c_{10} = 4 - c_1^2 \tag{14} $$

$$ c_{11} = 4 c_4 - 2 c_1 c_3 \tag{15} $$

$$ c_{12} = 4 c_5 - c_3^2 \tag{16} $$

$$ c_{13} = \frac{c_{11} - \sqrt{c_{11}^2 - 4 c_{10} c_{12}}}{2 c_{10}} \tag{17} $$

$$ c_{14} = \frac{\sqrt{c_{11}^2 - 4 c_{10} c_{12}}}{c_{10}} \tag{18} $$

$$ c_{15} = \sqrt{c_{10} c_{14}} \tag{19} $$

$$ c_{16}(s) = c_1 c_{13} - c_3 - 2 s \tag{20} $$

$$ c_{17}(s) = \frac{-c_{15} + \sqrt{c_{15}^2 - 4 |c_{16}(s)|^2}}{2 i c_{16}(s)} \tag{21} $$

$$ c_{18}(s) = \frac{-c_{15} - \sqrt{c_{15}^2 - 4 |c_{16}(s)|^2}}{2 i c_{16}(s)} \tag{22} $$
...
<img src="./assets/lwifig1.gif" alt="">
