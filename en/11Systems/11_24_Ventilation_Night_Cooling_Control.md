...
*Min Inlet Air:* The Min Inlet Air parameter sets a lower limit for the supply temperature. In this way the control function simulates a supply sensor that overrides the room sensor when the supply temperature drops below the set point for the minimum sensor. If there is sufficient capacity in the heat recovery unit and heating coil, and if they are active, the control function will keep the supply temperature at the minimum value.
...
*   If there is no "<span id="green_text">v</span>" next to Fans, it means that no heat is transferred from the engines to the inlet air when the system controls according to this night cooling strategy. The engines still run, forcing the air through the system.
