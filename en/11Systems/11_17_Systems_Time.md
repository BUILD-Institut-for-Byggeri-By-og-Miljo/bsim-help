...
# Systems, *Time*
All forms of schedules for systems include one or more time definitions. Each time definition is linked to a single control action and specifies when the system is to implement that control action.
...
The operating period for a ventilation system can, for example, be described by means of a time definition in which omitted weeks could correspond to the Christmas, Easter, summer or autumn holidays. The days correspond to all weekdays (Monday to Friday), and a time definition might, for example, be normal school hours plus four hours of evening classes.
...
By default, a time definition belongs to the first tariff class (Tariff 0).
