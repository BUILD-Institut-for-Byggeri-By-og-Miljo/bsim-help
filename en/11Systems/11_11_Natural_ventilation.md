...
Natural ventilation is implemented as a special kind of Venting (airing) in an expansion module of BSim, and is based on [By og Byg (SBi) Direction 202](../20The_Mathematical_basis/20_13_Extract_from_by_og_byg_direction_202.md), Natural ventilation in commercial buildings, Calculation and dimensioning (2002).
...
*   *Natural Ventilation:* The desired model is selected from the selection menu found under Natural Ventilation. Above the list the name of the model selected by BSim is shown. The following options are available: 
    ...
    *   *(Automatic):* BSim selects the model to use from the geometry of the room, see the [mathematical background](../20The_Mathematical_basis/20_12_Natural_ventilation.md) for illustrations of model geometries equal to the selections shown below.
