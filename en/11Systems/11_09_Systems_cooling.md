...
*Unit:* By placing a "tick" mark next to *Unit,* it is possible to change from inputting the absolute amount of installed power in the thermal zone to inputting the amount of installed power per m² floor area. This is especially useful if the same heating system is to be used (copied) in more thermal zones with varying heat loss.
...
Central Cooling System indicates that the cooling power for the system comes from a central cooling system. The central cooling system cannot be activated as a source for the cooling system until the external program PackCalc has been installed. [PackCalc](../24Miscellaneous/24_01_PackCalc_Cooling.md) has been developed by IPU Technology Development at the Danish Technical University (DTU).

Cooling data opens a dialog for input data for the cooling system.

In the [schedule](11_02_Systems_schedule.md) a list of connected sets of controls and time definitions are found.

See also:
*   Tab *[HeatCoolCtrl](../24Miscellaneous/24_45_Cooling_control.md)*
