...
To any WinDoor in a BSim model it is possible to connect a solar shading device, which is described by the input fields shown in the figure below. The solar shading device itself is described by a limited number of parameters that define the physical properties of the device, as shown in the table below.

<figure id="center_img">
<img src="./assets/shading.gif" alt="Dialog box for defining shading device for a WinDoor. Values for Shading Coeff. and Max Sun are only used if no values are given in any of the control strategies. Data for Slat Width and Slat Dist. are only used for devices of the type Venetian blind.">
<figcaption>Dialog box for defining shading device for a WinDoor. Values for Shading Coeff. and Max Sun are only used if no values are given in any of the control strategies. Data for Slat Width and Slat Dist. are only used for devices of the type Venetian blind.</figcaption>
</figure>


Data in BSim for the dialog SolarShading. Parameters in grey fields are only used for special types of shading devices.
...
The functionality of solar shading devices is, as with any system in BSim, described in a [schedule](11_02_Systems_schedule.md), where for one or more [time](11_17_Systems_Time.md) definitions it is possible to define which control to use. An unlimited number of time schedules can be defined with corresponding control strategies for the solar shading device.

The following four control strategies can be selected:
...
Table values offer the possibility of using detailed information about the slats in the shading system by attaching information from an [external text file](../24Miscellaneous/24_32_Standard_tables_for_slate_based_shading_systems.md). When clicking the button, a dialog opens to find the file on the PC.
