...
The buildings of the model can be located with faces having a negative z-value. This does **not** mean that the model is below the surface of the ground, even though it looks like it in the spatial view. Solar radiation will thus still be able to come through windows located all or partially beneath the origin of the co-ordinate system.
...
*   Constructions between two rooms are drawn symmetrically around the system line.
...
In all the programs in the *BSim* suite it is possible to call up a menu by clicking the right mouse button. The menu is different in the various programs ([SimView](../09SimView/09_01_SimView.md), [XSun](../14XSun_Analysis_of_incident_solar_radiation/14_01_Analysis_of_incident_solar_radiation_with_XSun.md), [tsbi5](../13tsbi5_thermal_simulation/13_01_tsbi5.md) and [SimLight](../15SimLight_Daylight_calculations/15_01_Daylight_calculations_with_SimLight.md)) and contains the most frequently used functions for the current location.
